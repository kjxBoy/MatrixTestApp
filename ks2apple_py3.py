...
@LastAuthor: Upgraded to Python 3
'''
...
import os
import sys
import html  # Python 3: cgi.escape -> html.escape
import argparse  # Python 3: optparse -> argparse (更推荐)
import traceback
...
def get_cpu_type(arch):
    return {'arm64': 'ARM-64',
            'arm': 'ARM',
            'x86': 'X86',
            'x86_64': 'X86_64'}.get(arch, 'Unknown')
...
def get_time(report):
    ...
    if isinstance(timestamp, int):  # Python 3: type() == type(0) -> isinstance()
...
def parse_system_info(report):
    ...
    headers = ['System Info: {']
    device = _s('machine')
    if device in device_map:  # Python 3: has_key() -> in
    ...
    mem = sys.get("memory")
    if mem:
        fmt = lambda x: "    Mem {0:6}:  {1:4} M".format(x, int(mem[x])//1024//1024)  # Python 3: 整除
...
def parse_error_info(report):
    ...
    if not error:
        print("warning: no error found in crash")  # Python 3: print()
        return []
    mach = error['mach']
    signal = error['signal']
...
def parse_backtrace(report, backtrace):
    ...
    for trace in backtrace.get('contents', []):
        ...
        if not img:
            print("error, no img found for pc: %s" % pc)  # Python 3: print()
    ...
    for image in images:
        try:
            ...
            if False:
                ...
                if name == 'WeChat':
                    print('{0:>#18x} - {1:>#18x} {2}{3}  <{4}> {5}'.format(addr, addr+size-1, is_base, name,uuid, path))  # Python 3: print()
...
def parse_mem_info(report):
    ...
    fmt = lambda x: "    {0:6}: {1:4} M,".format(x, int(mem[x])//1024//1024)  # Python 3: 整除
...
def parse_other_info(report):
    ...
    user_info = report.get("user", None)
    if not user_info or not user_info.get(APP_NAME, None) or not user_info[APP_NAME].get("log"):
        print("no log found...")  # Python 3: print()
        return result 
...
def ks_json_2_apple(report, fout):
    ...
    try:
        ...
        errors = []
    for line in errors:
        line = html.escape(line)  # Python 3: cgi.escape -> html.escape
        fout.write(line+'\n')
...
if __name__ == '__main__':
    # Python 3: 使用 argparse (更推荐)
    parser = argparse.ArgumentParser(description='Convert Matrix/KSCrash JSON report to Apple crash format')
    parser.add_argument("-i", "--input_file", required=True, help="Input Matrix/KSCrash JSON file")
    parser.add_argument("-o", "--output_file", required=True, help="Output crash report file")
    ...
    with open(args.input_file, 'r', encoding='utf-8') as f:  # Python 3: 显式指定编码
        reports = json.load(f)
        
    with open(args.output_file, 'w', encoding='utf-8') as fout:  # Python 3: 显式指定编码
        if isinstance(reports, list):  # Python 3: type() is list -> isinstance()
            print("This input file contains %d report(s)" % (len(reports)))
