...
```mermaid
graph TB
    A[backtrace 获取虚拟地址<br/>0x102abc100] --> B{选择符号化方式}
    
    B -->|运行时| C[dladdr 查询]
    C --> D[查询动态链接器<br/>_dyld_get_image_header]
    D --> E[返回 Dl_info<br/>函数名: testScenario2]
    
    B -->|离线| F[准备 dSYM 文件]
    F --> G[获取加载地址<br/>0x102000000]
    G --> H[调用 atos 工具]
    H --> I[返回完整信息<br/>testScenario2<br/>TestAsyncCPUViewController.mm:45]
